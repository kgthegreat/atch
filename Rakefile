  ...
  task :export_products => :environment do
    ...
    FasterCSV.open("#{RAILS_ROOT}/tmp/products.csv", "w") do |csv|
      ...
      products.each do |p|
        ...
        count_i = count_i + 1
        puts "Exporting #{p.name.titleize} at no. #{count_i}"
        csv << [p.id,
                p.sku,
                p.name.titleize,
                p.description,
...
=begin
          ...
          #puts "weicht " + weight_option_value.name + weight_option_value.presentation #+ weight_option_value.option_type.name
          if weight_option_value.nil?
          puts "inside if"
          weight_option_value = OptionValue.create(:name => measure,
                                                   :presentation => measure,
                                                   :option_type => weight_type
                                                   );
          end
 variant.option_values << weight_option_value
=end
      ...
      if id.nil? && !name.nil?   # a variant is not a new product. In the csv, the variant does not have the name specified.
        # Adding new product
        puts "Adding new product: #{name} at no. #{n}"
        product = Product.new()
      ...
      product.option_types = [weight_type]
      if !image_path.nil?
        img = Image.create(:attachment => File.open(image_path), :viewable => product)
      end


      if has_variants.upcase.include? "TRUE"
        puts "Yes we have some variants"
        total_variants = no_of_variants.to_i
        product_with_variant = product
        product.save!
      elsif has_variants.upcase.include? "FALSE"
        product.save!
      end
